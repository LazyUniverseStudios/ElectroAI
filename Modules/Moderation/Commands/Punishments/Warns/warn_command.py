...
async def warn_command(ctx, target: discord.Member = None, *, reason=None):
    ...
    if bot.guild_permissions.manage_messages or bot.guild_permissions.administrator:
        ...
        embed = embeds.Embed(title="Error", description="I do not have permission to warn members.", color=0xFF0000)
    ...
    if author.guild_permissions.manage_messages or author.guild_permissions.administrator:
        ...
        embed = embeds.Embed(title="Error", description="You do not have permission to warn members.", color=0xFF0000)
        await ctx.send(embed=embed)
        return
    
    if target == None:
        embed = embeds.Embed(title="Error", description="Please specify a user to warn.", color=0xFF0000)
        await ctx.send(embed=embed)
        return
    ...
    if isinstance(target, discord.Member):
        if bot.top_role <= target.top_role:
            embed = embeds.Embed(title="Error", description="I cannot warn this user because they have a higher or equal role than me.", color=0xFF0000)
            await ctx.send(embed=embed)
            return
        
        if author.top_role <= target.top_role and author != ctx.guild.owner:
            embed = embeds.Embed(title="Error", description="You cannot warn this user because they have a higher or equal role than you.", color=0xFF0000)
            await ctx.send(embed=embed)
            return
    
    if target == ctx.guild.owner:
        embed = embeds.Embed(title="Error", description="You cannot warn the server owner.", color=0xFF0000)
        await ctx.send(embed=embed)
        return
