...
async def kick_command(ctx, target: discord.Member = None, *, reason=None):
    ...
    if author.guild_permissions.kick_members or author.guild_permissions.administrator:
        ...
        embed = embeds.Embed(title="Error", description="You do not have permission to kick members.", color=0xFF0000)
        await ctx.send(embed=embed)
        return
    
    
    if target == None:
        embed = embeds.Embed(title="Error", description="Please specify a user to kick.", color=0xFF0000)
        await ctx.send(embed=embed)
        return
    ...
    if isinstance(target, discord.Member):
        ...
        if author.top_role <= target.top_role and author != ctx.guild.owner:
            embed = embeds.Embed(title="Error", description="You cannot kick this user because they have a higher or equal role than you.", color=0xFF0000)
            await ctx.send(embed=embed)
            return
    
    if target == ctx.guild.owner:
        embed = embeds.Embed(title="Error", description="You cannot kick the server owner.", color=0xFF0000)
        await ctx.send(embed=embed)
        return
    ...
    try:
        ...
        embed = embeds.Embed(title="Error", description=f"An error occurred while trying to kick the user: {str(e)}", color=0xFF0000)
        await ctx.send(embed=embed)
        return