...
from modules.economy.db.claim_MonthlyReward import claimMonthlyReward

@commands.command(name="monthlyreward", aliases=["monthly", "mr"])
async def monthly_reward(ctx):
    """
    Claims the monthly reward for the user.

    Args:
        ctx: The context of the command.
    """
    user_id = ctx.author.id
    success, next_use_time = await claimMonthlyReward(user_id)
