...
async def ban_command(ctx, target: discord.User = None, *, reason=None):
    ...
    if bot.guild_permissions.ban_members or bot.guild_permissions.administrator:
        ...
        embed = embeds.Embed(title="Error", description="I do not have permission to ban members.", color=EmbedColor_Error)
    ...
    if author.guild_permissions.ban_members or author.guild_permissions.administrator:
        ...
        embed = embeds.Embed(title="Error", description="You do not have permission to ban members.", color=EmbedColor_Error)
        await ctx.send(embed=embed)
        return
    
    if target == None:
        embed = embeds.Embed(title="Error", description="Please specify a user to ban.", color=EmbedColor_Error)
        await ctx.send(embed=embed)
        return
    ...
    if isinstance(target, discord.Member):
        if bot.top_role <= target.top_role:
            embed = embeds.Embed(title="Error", description="I cannot ban this user because they have a higher or equal role than me.", color=EmbedColor_Error)
            await ctx.send(embed=embed)
            return
        
        if author.top_role <= target.top_role and author != ctx.guild.owner:
            embed = embeds.Embed(title="Error", description="You cannot ban this user because they have a higher or equal role than you.", color=EmbedColor_Error)
            await ctx.send(embed=embed)
            return
    
    if target == ctx.guild.owner:
        embed = embeds.Embed(title="Error", description="You cannot ban the server owner.", color=EmbedColor_Error)
        await ctx.send(embed=embed)
        return
    ...
    try:
        ...
        embed = embeds.Embed(title="Error", description=f"An error occurred while trying to ban the user: {str(e)}", color=EmbedColor_Error)
        await ctx.send(embed=embed)
        return
